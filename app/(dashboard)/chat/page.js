...
export default function RetroChatRoom() {
  return (
    /* 
      1. w-full: Blows the container wide open to utilize 100% of the remaining screen space.
      2. h-[calc(100vh-4rem)]: Calculates exact space to fit top-to-bottom right next to your sidebar.
      3. rounded-none border-t-0 border-b-0: Marries the card flush to the outer viewport bounds.
    */
    <Card
      variant="brutal"
      className="w-full h-[calc(100vh-4rem)] border-4 border-black bg-white shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] flex flex-col overflow-hidden p-0 rounded-none"
    >
      {/* Top Header Information Panel Bar (Now spans edge-to-edge) */}
      ...
      {/* Middle Scrollable Layout Conversation Feed (Spreads text boxes wide) */}
      <div className="flex-1 p-6 overflow-y-auto bg-[#F1EFE6] space-y-6 font-mono text-xs">
        ...
        <div className="flex gap-3 max-w-[90%]">
          <div className="border-2 border-black bg-yellow-300 p-2.5 h-fit shrink-0 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
            <Terminal className="w-4 h-4" />
          </div>
          <Card
            variant="brutal"
            className="bg-white border-2 border-black p-5 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] font-sans space-y-2 text-sm text-black rounded-none flex-1"
          >
        ...
        <div className="flex gap-3 max-w-[90%] ml-auto justify-end">
          <div className="border-2 border-black bg-cyan-300 p-5 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] font-sans text-sm text-black font-semibold">
      ...
      {/* Control Navigation Input Footer Row (Stretches across full wide bottom) */}
      <div className="border-t-4 border-black p-4 bg-white flex items-center gap-3 shrink-0">
        <Button
          variant="outline"
          className="border-2 border-black p-3 bg-slate-100 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] h-11 w-11 flex items-center justify-center shrink-0"
        >
